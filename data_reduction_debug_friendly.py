...
def import_data(a):
    filename = a#raw_input("Enter "+a+"-file number\n")   
...
def make_cal_im(a,b):
    number1 = int(a)#int(raw_input("first file number "+a+"\n"))
    number2 = int(a)#int(raw_input("last file number "+a+"\n"))
...
# the right filter that has already been devided by the exposure time of the flat field.
def calibrate(im, im_t, bias, flat):
    ...
    # for the sky annulus
    [flux, fluxerr, sky, skyerr] = sp.aper(image=im, xc=x, yc=y, phpadu=[5], \
    apr=[5], skyrad=[10,15], badpix=[0,0], flux=True,nan=True)
    return(flux, fluxerr)
...
raw_R, t_R = import_data(str(1295042))#"R")
raw_V, t_V = import_data(str(1295045))#"V")
bias = make_cal_im(str(1294990),False)#"bias",False)
flat_R = make_cal_im(str(1294980),True)#""flat_R",True)
flat_V = make_cal_im(str(1294980),True)#""flat_V",True)
